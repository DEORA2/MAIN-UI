...
function espLibrary.getHealth(player, character)
    ...
    if (humanoid) then
        return math.round(humanoid.Health), humanoid.MaxHealth;
    end
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or (Enum.RenderPriority.Camera.Value + 1), function()
        for player, objects in next, self.espCache do
            ...
            if (character and torso) then
                ...
                objects.side.Position = round(position + vector2New(size.X + 3, -(objects.top.TextBounds.Y - 5)));

                local curWeap
                local e = pcall(function()
                    curWeap = character.EquippedTool.Value;
                end)
                if not e then
                    curWeap = "None"
                end

                objects.side_two.Visible = show and self.options.showWeapon;
                objects.side_two.Font = self.options.font;
                objects.side_two.Size = self.options.fontSize;
                ...
                objects.bottom.Position = round(position + vector2New(size.X * 0.5, size.Y + 20));
        ...
        for player, highlight in next, self.chamsCache do
            ...
            if (character and torso) then
                ...
                highlight.DepthMode = self.options.chamsVisibleOnly and Enum.HighlightDepthMode.Occluded or Enum.HighlightDepthMode.AlwaysOnTop;
